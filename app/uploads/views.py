...
GENESIS = BASE_DIR + 'mediafiles/accounts'

def getgenesis(pro):
    genesis = BASE_DIR + '/mediafiles/accounts/' + str(pro.user.email) + '/genesis/'
    return genesis
def range(min=5):
    return range(min)
...
def firstfolder(request):
    ...
    path = BASE_DIR+'/mediafiles/'+ str(user)
    if pro.gid == 0:
        os.chdir(path)
        os.mkdir('genesis')
        path += '/genesis/'
...
def rootfolder(request):
    ...
    if foldername == '':
        ...
        return render(request, 'uploads/my-drive.html', context)
    path = BASE_DIR+'/mediafiles/accounts/'+ str(user)
    if pro.gid == 0:
...
def upload_driver(request):
    ...
    if upload_file:
        ...
        target_folder = BASE_DIR+'/mediafiles/accounts/'+str(request.user)+'/genesis/'
        if not os.path.exists(target_folder): os.mkdirs(target_folder)
        ...
        f = File.objects.create(owner=pro, name=filename.strip(), file_type=ftype.lower(),path='/mediafiles/accounts/'+str(request.user)+'/genesis/'+filename)
